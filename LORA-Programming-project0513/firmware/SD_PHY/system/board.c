...
//    //
    ...
    //RF SWITCH
    GpioInit(&SD_PHY.SWITCH_CTRL.CTRL_SWITCH_A,    PIN_SW_A.portOut,  PIN_SW_A.portIn,   PIN_SW_A.portDir,  PIN_SW_A.pin,  PIN_SW_A.portNumber,   PIN_OUTPUT, true);
    GpioInit(&SD_PHY.SWITCH_CTRL.CTRL_SWITCH_B,    PIN_SW_B.portOut,  PIN_SW_B.portIn,   PIN_SW_B.portDir,  PIN_SW_B.pin,  PIN_SW_B.portNumber,   PIN_OUTPUT, false);
#endif
...
void SetSwitchMode (RF_Switch_Mode_t mode) {
    if (mode == OTA_MODE) {
        GpioWrite(&SD_PHY.SWITCH_CTRL.CTRL_SWITCH_A, 0);
        GpioWrite(&SD_PHY.SWITCH_CTRL.CTRL_SWITCH_B, 1);
    } else if (mode == SDPHY_MODE) {
        GpioWrite(&SD_PHY.SWITCH_CTRL.CTRL_SWITCH_A, 1);
        GpioWrite(&SD_PHY.SWITCH_CTRL.CTRL_SWITCH_B, 0);
    }
}
